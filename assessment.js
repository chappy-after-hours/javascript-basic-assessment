...
for (var i = 0; i < heroes.length; i++) {
	if (heroes[i].toLowerCase() === 'flash') {
		secondHero = heroes.splice(i,1).join();
	}
}
// leave batman in the array but put a copy of him on a variable called thirdHero
var batmanIndex = heroes.indexOf('batman');
var thirdHero = heroes.slice(batmanIndex,batmanIndex+1).join();
...
function coderTest(person) {
	if (person.name.toLowerCase() === 'Jeremy'.toLowerCase()) {
		person.lovesCode = 10;
	} else if (person.name.toLowerCase() === 'Brack'.toLowerCase()) {
		person.lovesCode = 0;
	} else {
		person.lovesCode = 5;
	}
